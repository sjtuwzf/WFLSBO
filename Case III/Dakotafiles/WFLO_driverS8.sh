...
awk 'END { print $NF }'  ~/wzf/MSMD_add/S8/postProcessing/totalpower/0/volRegion.dat > ~/wzf/MSMD_add/AEP/S8_1.dat;
...
mpirun -np 60 renumberMesh -overwrite -parallel;
mpirun -np 60 actuatorDiskExplicitForceSimpleFoam -parallel;
awk 'END { print $NF }'  ~/wzf/MSMD_add/S8/postProcessing/totalpower/0/volRegion.dat > ~/wzf/MSMD_add/AEP/S8_2.dat;
